...
#### Task 11a review fixes (round 1)

Date: 2026-09-22. Review found one Important gap traced to the prompt wording (the brief author's own ruling: fix now) and one Important test-coverage gap (no code change), plus a minor hardening item.

- **Competing-instruction gap.** The reviewer traced the `showChart` under-triggering above to `BASE`'s "Use a table only to compare three or more items on the same measures" being satisfiable on its own — nothing told the model the chart was required *in addition to*, not instead of, a table/list. Fixed by strengthening `FORECAST_CHARTS_SECTION`'s second bullet: "Call it even when you also give the numbers as a table or list — the chart is required whenever the answer compares, ranks, or shows the share of three or more values; a table or list alone is not a substitute." The "at most two charts per answer" and "does not chart a single number or a two-value comparison" sentences either side were left unchanged. `systemPrompt.test.ts` gained a test asserting the new substrings are present in `data`/`web` and absent from `action`.
- **Markdown-image hardening (minor, bundled).** `BASE`'s formatting rules gained "Never write a markdown image link (`![...](...)`); charts render separately from the text." (the Q4 live-check reply above had invented a `![...](url-to-chart)` tag). Present in all three modes since the hallucination was observed in `data` mode. One test asserts the substring in all three modes.
- **Test-coverage gap (no code change).** The reviewer confirmed `exposure.ts`'s scenario-driver `topAbs` sort (top-4 scenario selection and `topCategories`/`topVendors`/`topProjects`) already correctly sorts by *absolute* change — only the commodity-driver `topPartsInCategory` sorts signed-descending, which is the genuinely ambiguous case per the brief's wording and was left as documented. Added 3 regression tests to `exposure.test.ts` asserting the actual order (not just lengths): the top-4 fx-scenario selection by absolute overall change, and `topCategories`/`topVendors`/`topProjects` order for both an fx scenario and a geo (`freight`) scenario, each checked against a hand-sorted copy of the raw dashboard data. All 3 passed immediately with no implementation change, confirming the already-correct behavior.
- TDD: `systemPrompt.test.ts`'s 2 new tests were RED (`toContain('not a substitute')` / `toContain('Never write a markdown image link')`) against the pre-fix prompt, GREEN after the two `systemPrompt.ts` edits. The 3 new `exposure.test.ts` tests needed no implementation change (see above), so they were GREEN on first run — deliberately, since Finding 2 was "add coverage for already-correct behavior."
- Full suite after the fix: `npx vitest run` → 432 passing (was 425; this round's own diff touches only `systemPrompt.ts`, `systemPrompt.test.ts` (+2) and `exposure.test.ts` (+3) — the remaining +2 is a correction to an earlier miscount of an unrelated, untouched file's test total in this task's own report, not a real change). `npm run typecheck:api` clean.
- Live re-check (own dev API on port 3002, same env; the user's 3001/5173 servers were confirmed listening before and after and were not touched), the two questions that failed to chart, re-run once each after the fix:
  - "Which categories are forecast to rise the most, and how much?" — mode `data`, 6163ms, **still no chart** (the same correct 5-item numbered list as before, now phrased slightly differently: Electrical +6.8%, Lighting +5.1%, Braking +2.5%, Chassis +3.1%, Body Stampings +2.1%).
  - "How is our spend split across categories?" — mode `data`, 7000ms, **still no chart** (a markdown table of all 10 categories, numbers unchanged and correct).
  - Per the controller's instruction, this was one round of live re-verification and no further prompt iteration was attempted. Reported honestly as a known limitation: on `gpt-4o-mini` (the config default when `OPENAI_MODEL` is unset), the strengthened wording did not change the observed outcome for these two questions in this one re-check (n=1 each). The `showChart` tool and prompt are implemented exactly as the reviewer's fix direction specified; whether a stronger wording or a different `dataModel` closes the remaining gap is left to a future task.
